...
def sidebar(
    costo_mix_defecto: float,
    costo_fijo_defecto: float,
    costo_millar_defecto: float,
    load_capacity_default: float,
    percentage_feed_default: int,
    percentage_sob_default: int,
    storage,
    dias_secado: int,
) -> None:
    """
    Esta ``función`` muestra el sidebar de configuraciones de la aplicación

    Args:
        costo_mix_defecto (float): costo mix por defecto del campo
        costo_fijo_defecto (float): costo fijo por defecto del campo
        costo_millar_defecto (float): costo millar por defecto del campo
        load_capacity_default (float): capacidad de carga por defecto del campo
        percentage_feed_default (int): valor con el aumento/disminucion por defecto
        del aliemento cuando se se usa proyeccion dinamica del alimento
        percentage_sob_default (int): valor con el aumento/disminucion por defecto
        de la sob cuando se se usa ya sea por consumo o por campo.
        dias_secado (int): dias secado por defecto del campo.
    Returns:
        Retorna None
    """
    ...
    st.session_state.costo_larva = st.number_input(
        "Ingrese Costo millar larva",
        min_value=0.0,
        max_value=30.0,
        value=costo_millar_defecto,
        step=0.1,
    )
    ...
    st.session_state.dias_secos = st.number_input(
        "Ingrese Días Secos", min_value=0, max_value=30, value=dias_secado, step=1
    )
    ...
    st.session_state.percentage_dynamical_sob = st.slider(
        "Ingrese un porcentaje de aumento o disminución de la sob: ",
        -10,
        10,
        percentage_sob_default,
    )
